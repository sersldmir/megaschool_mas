...
def format_feedback(feedback: dict) -> str:
    ...
    if feedback.get('recommendation_reasoning'):
        lines.append(f"\nОбоснование: {feedback['recommendation_reasoning']}")
...
def main():
    ...
    max_questions_str = input("Максимальное количество вопросов (по умолчанию 10): ").strip()
    try:
        max_questions = int(max_questions_str) if max_questions_str else 10
    except ValueError:
        max_questions = 10
    ...
    while state['phase'] != InterviewPhase.FINISHED and iteration < max_iterations:
        ...
        try:
            ...
            if state['phase'] == InterviewPhase.WAITING_FOR_ANSWER:
                print(f"\n[INTERVIEWER] Интервьюер: {state['current_question']}")
                ...
                internal_thoughts = f"[Observer]: {observer_thought} [Interviewer]: {interviewer_instruction}"
